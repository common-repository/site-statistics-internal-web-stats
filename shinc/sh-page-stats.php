...
<div class="sh-left sh-w-30">
    <h3><?php echo __( 'Top 10 Pages', 'sh-language' );?></h3>

    <div class="sh-table-block">
    <?php 
    
    $toptenvisitors = $wpdb->get_results("SELECT *,COUNT(`p_id`) AS CNT FROM " . SH_WEB_STATS_DETAILS . " WHERE `tracking_id`!='' AND p_type='page' GROUP BY `p_id` ORDER BY CNT DESC  LIMIT 10", ARRAY_A);
...
<div class="sh-table-block">
    ...
    $recentvisitors = $wpdb->get_results("SELECT * FROM " . SH_WEB_STATS_DETAILS . " WHERE `tracking_id`!='' AND p_type='page' ORDER BY `capture_date` DESC  LIMIT 10", ARRAY_A);
